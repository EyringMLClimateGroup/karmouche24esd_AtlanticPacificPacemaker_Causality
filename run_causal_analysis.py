...
class run_obs_analysis:
    ...
    """
    @staticmethod
    def calculate_obs_indices(ds_ts_obs, ds_psl_obs, ds_ua_obs, ds_ts_em, ds_psl_em, ds_ua_em, utils):
        """
        ...
        _, raw_zonal_wind_pacific_obs = (
            utils.extract_reg_timeseries1(ds_ua_obs["ua"], 
                ds_em=None, 
                minlon=180, 
                maxlon=210, 
                minlat=-6,
        ...
        if np.corrcoef(pdv_obs_dict["pdv_timeseries"], nino34_obs_dict["nino34_timeseries"])[0, 1] < 0:
            pdv_obs_dict["pdv_timeseries"] *= -1
        if np.corrcoef(pdv_obs_dict_iso["pdv_timeseries"], nino34_obs_dict["nino34_timeseries"])[0, 1] < 0:
            pdv_obs_dict_iso["pdv_timeseries"] *= -1
        if np.corrcoef(pdv_obs_dict_veryraw["pdv_timeseries"], nino34_obs_dict["nino34_timeseries"])[0, 1] < 0:
            pdv_obs_dict_veryraw["pdv_timeseries"] *= -1

        # Prepare dataframe for Tigramite's PCMCIplus
        tig_data = np.zeros((len(naoo_obs_seas), 6))
        tig_data[:, 0] = (
            tna_obs_dict["tna_timeseries"].resample(time="QS-DEC", keep_attrs=True).mean()
        )
        ...
        tig_data[:, 5] = alt3["atl3_index"].resample(time="QS-DEC", keep_attrs=True).mean()

        # Dataframe for the isolated internal variability
        ...
        return tig_data, tig_data_iso, amv_obs_dict, amv_obs_dict_iso, amv_obs_dict_veryraw, pdv_obs_dict, pdv_obs_dict_iso, pdv_obs_dict_veryraw,yearsarrobs, lenP1, lenP2
    
    @staticmethod
    def run_analysis_obs(pcmci, exp, list_titleperiods, FIGS_DIR,
                         var_names=None, 
                         min_tau=0, max_tau=4, pc_alpha=None,
                         node_pos=None,test_assumption=None) :
        """
        ...
        N=len(var_names)
        # Here we implement our assumptions. For more about build_link_assumptions see the code here tigramite/tigramite/pcmci_base.py
        ...
        tp.plot_graph(
            ...
            save_name=FIGS_DIR+f"pcmciplus_graph_with_assumptions_OBS_{list_titleperiods[exp-1].replace(' ', '_')}.png",
            figsize=(8, 4),
            show_colorbar=False,
        )
        plt.show()
...
class run_sliding_window:
    ...
    @staticmethod
    def load_and_calculate_indices_PACIFIC(ensm, ds_ts_em, ds_ua_em, ds_psl_em,ts_nc,ua_nc,psl_nc, T, N):
        """ 
        ...
        naoo_seas, _ = utils.calculate_nao(
            ds_psl_pac["PSL"], None, seasonal=True
        )
        pnaa_seas, _ = utils.calculate_pna(
            ds_psl_pac["PSL"], None, seasonal=True
        )
        ...
        _, raw_zonal_wind_pacific_iso = (utils.extract_reg_timeseries1(
                ds_ua_pac["U"], ds_em=ds_ua_em["ua"], minlon=180, 
            maxlon=210, 
            minlat=-6,
            maxlat=6,
            seasonal=True,
        ...
        tig_data_PAC[:, 2] = (
            nino34_dict["nino34_timeseries"].resample(time="QS-DEC", keep_attrs=True).mean()
        )
        ...
        tig_data_PAC_iso[:, 0] = (
            tna_dict_iso["tna_timeseries"].resample(time="QS-DEC", keep_attrs=True).mean()
        )
        ...
        amv_dict_veryraw = utils.calculate_amv(ds_ts_pac['TS'], remove_gm=False, period=perioda)
        amv_dict_iso = utils.calculate_amv(ds_ts_pac['TS'], ds_ts_em['ts'], remove_gm=False, period=perioda)
        pdv_dict = utils.calculate_pdv(ds_ts_pac['TS'], None, period=perioda)
        pdv_dict_veryraw = utils.calculate_pdv(ds_ts_pac['TS'], remove_gm=False, period=perioda)      
        pdv_dict_iso = utils.calculate_pdv(ds_ts_pac['TS'], ds_ts_em['ts'], remove_gm=False, period=perioda)
        yearsarrobs = pnaa_seas_iso[1:]['time.year'].values

        return ensm-1, tig_data_PAC, tig_data_PAC_iso, amv_dict, amv_dict_iso, amv_dict_veryraw, pdv_dict, pdv_dict_iso, pdv_dict_veryraw, yearsarrobs

    
    @staticmethod
    def load_and_calculate_indices_ATLANTIC(ensm,ds_ts_em,ds_ua_em,ds_psl_em,ts_nc,ua_nc,psl_nc,T,N):    
        """ 
        ...
        ds_ua_atl=xr.open_dataset(ua_nc).sel(lev=925, method='nearest').sel(time=slice('1950-01-01', '2013-12-01'))
        ...
        ds_ts_atl,ds_ua_atl,ds_psl_atl= ds_ts_atl.interp(lon=ds_ts_em['lon'], lat=ds_ts_em['lat']),\
            ds_ua_atl.interp(lon=ds_psl_em['lon'], lat=ds_psl_em['lat']),\
                ds_psl_atl.interp(lon=ds_psl_em['lon'], lat=ds_psl_em['lat'])

        tna_dict, nino34_dict= utils.calculate_tna(ds_ts_atl['TS'],None, remove_gm=False), utils.calculate_nino34(ds_ts_atl['TS'],None, remove_gm=False)
        ...
        _,raw_zonal_wind_pacific=(utils.extract_reg_timeseries1(ds_ua_atl['U'], ds_em=None, minlon=180, maxlon=210, minlat=-6,maxlat=6,seasonal=True,))
        alt3=utils.calculate_ATL3_index(ds_ts_atl['TS'], None, remove_gm=False, period=['1950-01-01','2014-12-01'])#['tna_pattern_lp']

        tna_dict_iso, nino34_dict_iso = utils.calculate_tna(ds_ts_atl['TS'],ds_ts_em['ts'], remove_gm=False),  utils.calculate_nino34(ds_ts_atl['TS'],ds_ts_em['ts'], remove_gm=False)
        naoo_seas_iso,_=utils.calculate_nao(ds_psl_atl['PSL'], ds_psl_em['psl'], seasonal=True)
        pnaa_seas_iso,_=utils.calculate_pna(ds_psl_atl['PSL'], ds_psl_em['psl'], seasonal=True)
        _,raw_zonal_wind_pacific_iso=utils.extract_reg_timeseries1(ds_ua_atl['U'], ds_em=ds_ua_em['ua'],minlon=180, maxlon=210, minlat=-6,maxlat=6,seasonal=True,)   
        alt3_iso=utils.calculate_ATL3_index(ds_ts_atl['TS'], ds_ts_em['ts'], remove_gm=False,  period=['1950-01-01','2014-12-01'])#['tna_pattern_lp']           

        tig_data_ATL=np.zeros((T,N))
        tig_data_ATL[:,0]=tna_dict['tna_timeseries'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]
        tig_data_ATL[:,1]=pnaa_seas[1:]
        tig_data_ATL[:,2]=nino34_dict['nino34_timeseries'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]
        tig_data_ATL[:,3]=raw_zonal_wind_pacific[1:]
        tig_data_ATL[:,4]=naoo_seas[1:]
        tig_data_ATL[:,5]=alt3['atl3_index'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]

        tig_data_ATL_iso=np.zeros(tig_data_ATL.shape)
        tig_data_ATL_iso[:,0]=tna_dict_iso['tna_timeseries'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]
        tig_data_ATL_iso[:,1]=pnaa_seas_iso[1:]
        tig_data_ATL_iso[:,2]=nino34_dict_iso['nino34_timeseries'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]
        tig_data_ATL_iso[:,3]=raw_zonal_wind_pacific_iso[1:]
        tig_data_ATL_iso[:,4]=naoo_seas_iso[1:]
        tig_data_ATL_iso[:,5]=alt3_iso['atl3_index'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]
        ...
        print('####################### ATL pac' + fensm)
        amv_dict = utils.calculate_amv(ds_ts_atl['TS'], None, period=['1950-01-01','2014-12-01'])
        amv_dict_veryraw = utils.calculate_amv(ds_ts_atl['TS'], remove_gm=False, period=['1950-01-01','2014-12-01'])
        amv_dict_iso = utils.calculate_amv(ds_ts_atl['TS'],ds_ts_em['ts'], remove_gm=False, period=['1950-01-01','2014-12-01'])
        pdv_dict = utils.calculate_pdv(ds_ts_atl['TS'], None, period=['1950-01-01','2014-12-01'])
        pdv_dict_veryraw = utils.calculate_pdv(ds_ts_atl['TS'], remove_gm=False, period=['1950-01-01','2014-12-01'])      
        pdv_dict_iso = utils.calculate_pdv(ds_ts_atl['TS'], ds_ts_em['ts'], remove_gm=False, period=['1950-01-01','2014-12-01'])
        yearsarrobs=pnaa_seas_iso[1:]['time.year'].values
        return ensm-1,tig_data_ATL, tig_data_ATL_iso, amv_dict, amv_dict_iso, amv_dict_veryraw, pdv_dict, pdv_dict_iso, pdv_dict_veryraw, yearsarrobs
    
    @staticmethod
    def run_analysis_sliding(pcmci,
                             var_names=None, 
                             min_tau=0, max_tau=4, pc_alpha=0.01,
                             window_step=40, window_length=80, 
                             subtitles=None, node_posld=None,test_assumption=None,
                             FIGS_DIR="figures/"):
        """
        ...
        method_args={'tau_min':min_tau, 'tau_max':max_tau, 'pc_alpha':pc_alpha,'link_assumptions': test_assumption}
        ...
        for w, subtitle in zip(range(n_windows), subtitles):
            ...
            tp.plot_graph(graphs[w], val_matrix=val_matrices[w], var_names=var_names, node_size=0.9, link_label_fontsize=14,node_label_size=18,node_pos=node_posld, save_name=None,
                        show_colorbar=False, vmin_edges=-1, vmax_edges=1, vmin_nodes=-1, vmax_nodes=1,
        ...
        if test_assumption is None:
            plt.savefig(FIGS_DIR+f"pcmciplus_sliding_window_analysis_pacem_with_NO_assumptions.png", format="png")
        else:
            plt.savefig(FIGS_DIR+f"pcmciplus_sliding_window_analysis_pacem_with_assumptions.png", format="png")
        plt.show()
        
        output_directory_sld = FIGS_DIR+"pcmciplus_sliding_window_singlewindow_analysis"
        os.makedirs(output_directory_sld, exist_ok=True)

        # Create and save individual figures
        for i, w in enumerate(range(n_windows)):
            lag_p_matrix = tp.setup_matrix(N=len(var_names), tau_max=max_tau, minimum=0., maximum=1, figsize=(5, 12), x_base=1, y_base=.1, tick_label_size=8, lag_units='seasons', plot_gridlines=True, var_names=var_names, legend_fontsize=15, label_space_left=.2)
            lag_p_matrix.add_lagfuncs(val_matrix=p_matrices[w], color='black')
            lag_p_matrix.add_lagfuncs(val_matrix=(np.where(p_matrices[w] <= pc_alpha, p_matrices[w], None)), color='red')
        ...
        if test_assumption is None:
            plt.savefig(FIGS_DIR+f"complete_lag_p_matrix_with_NO_assumptions.png", format="png")
        else:
            plt.savefig(FIGS_DIR+f"complete_lag_p_matrix_with_assumptions.png", format="png")
        plt.show()
    ...
    def build_atlantic_assumption(max_tau, pcmci):
        ...
        """
        link_assumptions_absent_link_means_no_knowledge = {5:{(2,0):'<?-'}, 2:{(5,0): '-?>'}} # ATL3 --> Nino34
        test_assumption = pcmci.build_link_assumptions(link_assumptions_absent_link_means_no_knowledge, 6, max_tau, 0)
    ...
    def build_pacific_assumption(max_tau, pcmci):
        ...
        """
        link_assumptions_absent_link_means_no_knowledge = {2:{(5,0):'<?-'}, 5:{(2,0): '-?>'}} # Nino34 --> ATL3
        test_assumption = pcmci.build_link_assumptions(link_assumptions_absent_link_means_no_knowledge, 6, max_tau, 0)
...
class run_picontrol:
    ...
    def load_calc_piControl(ds_ts_pic,ds_psl_pic,ds_ua_pic,peeriood,peerioodpsl):
        ...
        """
        amv_pic_dict,pdv_pic_dict=utils.calculate_amv(ds_ts_pic['ts'],remove_gm=False, period=peeriood),\
        ...
        if (np.corrcoef(pdv_pic_dict['pdv_timeseries'], nino34_pic_dict['nino34_timeseries'] )[0,1]) < 0:
            print(np.corrcoef(pdv_pic_dict['pdv_timeseries'], nino34_pic_dict['nino34_timeseries'] )[0,1])
            pdv_pic_dict['pdv_timeseries']*=-1

        tig_data_pic=np.zeros((len(naoo_pic_seas),6))
        tig_data_pic[:,0]=tna_pic_dict['tna_timeseries'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]
        tig_data_pic[:,1]=pnaa_pic_seas
        tig_data_pic[:,2]=nino34_pic_dict['nino34_timeseries'].resample(time='QS-DEC', keep_attrs=True).mean()[1:]
        tig_data_pic[:,3]=raw_zonal_wind_pacific_pic[1:]
        tig_data_pic[:,4]=naoo_pic_seas
        tig_data_pic[:,5]=alt3_pic['atl3_index'].resample(time='QS-DEC', keep_attrs=True).mean()[1:] 
    ...
    def make_regime_dict(amv_lp, pdv_lp, T, N):
        ...
        pdvplus_amvminus = np.where((pdvplus_pic + amvminus_pic == 0), 0, 1) # PDV+/AMV-
        pdvminus_amvplus = np.where((pdvminus_pic + amvplus_pic == 0), 0, 1) # PDV-/AMV+
        pdvminus_amvminus = np.where((pdvminus_pic + amvminus_pic == 0), 0, 1) # PDV-/AMV-
    ...
    @staticmethod
    def run_regime_masked_pcmciplus(regime, exp_data, var_names, regime_masks_dict, min_tau, max_tau, pc_alpha, figs_dir, scenario_id, link_assumption=True):
        """
        ...
        node_pos = {'x':x, 'y':y, 'transform':ccrs.PlateCarree(central_longitude=210)}  
        
        yrs_per_regime=np.count_nonzero(regime_masks_dict[regime][1000:2000,0] == 0)
        T, N = exp_data[:,:].shape
        # Create a dataframe
        dataframe = DataFrame(exp_data, var_names = var_names, mask=regime_masks_dict[regime][1000:2000])
        title = regime+' ('+str(yrs_per_regime)+' seasons)'
        # Run PCMCI+ algorithm
        pcmci = PCMCI(dataframe = dataframe, cond_ind_test = ParCorr(mask_type='y'))
        #link_assumptions_absent_link_means_no_knowledge = {4:{(0,0):'<?-'}, 0:{(4,0): '-?>'}} # NAO --> TNA
        if link_assumption == True:
            link_assumptions_absent_link_means_no_knowledge = {3:{(2,0):'<?-'}, 2:{(3,0): '-?>'}} # Nino34 --> PWCu
            test_assumption = pcmci.build_link_assumptions(link_assumptions_absent_link_means_no_knowledge, len(var_names), max_tau, 0)
        ...
        # Run PCMCI+ algorithm 
        results = pcmci.run_pcmciplus(tau_min=min_tau, tau_max = max_tau, pc_alpha =pc_alpha, link_assumptions = test_assumption)
        # Plot timeseries
        tp.plot_timeseries(dataframe = dataframe, grey_masked_samples='data')
        plt.show()
        # Plot lagged p-matrix 
        lag_p_matrix = tp.setup_matrix(N=N, tau_max=max_tau, minimum=0., maximum=1, figsize=(10,10), x_base=1, y_base=.1, tick_label_size=8, lag_units='seasons', \
                        plot_gridlines=True, var_names=var_names, legend_fontsize=15, label_space_left=.2,label_space_top=.05)
        lag_p_matrix.add_lagfuncs(val_matrix=results['p_matrix'], color='black')
        lag_p_matrix.add_lagfuncs(val_matrix=(np.where(results['p_matrix'] <= pc_alpha, results['p_matrix'], None)), color='red')
        # plot the PCMCI+ results (causal network)
        fig = plt.figure(figsize = (12, 15))
        ax = plt.axes(projection=ccrs.PlateCarree(central_longitude=210.))    # again the projection is defined for the axes, so that node_pos combines with the map
        ax.set_extent([-10, 150, -20,67], crs=ccrs.PlateCarree(central_longitude=210.))     # 
        # Additional features from cartopy
        ax.add_feature(cfeature.LAND,alpha=0.3)
        ax.add_feature(cfeature.COASTLINE,alpha=0.3)
        ax.set_title(title,size=20)
